...
if __name__ == "__main__":
    ...
    for index in range(len(filelist)):
        ...
        prediction,ts = test(leftname, rightname, savename)
        mask = np.logical_and(disp >= 0.001, disp <= opt.max_disp)

        error = np.mean(np.abs(prediction[mask] - disp[mask]))
        rate1 = np.sum(np.abs(prediction[mask] - disp[mask]) > opt.threshold) / np.sum(mask)        
        rate3 = np.sum(np.abs(prediction[mask] - disp[mask]) > opt.threshold*3) / np.sum(mask)        
        avg_error += error
        avg_rate1 += rate1
        avg_rate3 += rate3
        avg_ts += ts
        print(avg_ts/(index+1))
        print("===> Frame {}: ".format(index) + current_file[0:len(current_file)-1] + " ==> EPE Error: {:.4f}, Error Rate: {:.4f} {:.4f}".format(error, rate1, rate3))
    avg_error = avg_error / len(filelist)
    avg_rate1 = avg_rate1 / len(filelist)
    avg_rate3 = avg_rate3 / len(filelist)
    print("===> Total {} Frames ==> AVG EPE Error: {:.4f}, AVG Error Rate: {:.4f} {:.4f}".format(len(filelist), avg_error, avg_rate1, avg_rate3))
